# ScaleCases: Scalable Prediction Market Use Cases
 
## Project Overview
### Project name
ScaleCases: InvestorX, SurveyX, and NonRepudiationX (NRX) BlockChain Scalability Solution
...
&nbsp;&nbsp;&nbsp;LinkedIn: https://www.linkedin.com/in/mh.mazen/
...
A system that facilitate crowd opinion mining and incentivization for any survey that is made for any purpose. This not a prediction market use-case. But, it is a nice example of how some projects could be modeled and built with Gnosis Apollo packages as it is a prediction market use-case. 
 
 3) **NonRepudiationX**
 
 Is a BlockChain scalability solution that best fit for prediction market and can leverage Gnosis Apollo and exposes it to more use-cases. In addition to increasing the speed and lowing the cost payed by the end users.
...
***InvestorX:***
Almost all crypto community are investors who would appreciate good advice and could be engaged relatively easy in investment voting activities. There is already lots of Technical Analysts (on Telegram and specialized sites) with many followers and we will target them all.
...
Actually, we think that InvestorX and SurveyX, like almost all other use-cases that could be built with _Gnosis Apollo_, will stay under the hood, if thery were not be provided with a scalability solution. And we believe that NonRepudiationX is fairly simple, yet powerful enough, and best fit for prediction market. And it can attract developers and end-users to use Gnosis Apollo and the solutions built with.
...
90 000 USD for working 12 months on the three componets: _InvestorX_, _SurveyX_ and _NonRepudiationX (NRX) Scalability Solution_.

Actually, we can have the fund to only one selected item of the above (but the individual cost will be higher because of the learning curve). However, we strongly recommend that funding them 3 together. Especially that, InvestorX is built with Angular, SurveyX is built with React and the NRX will expose them to mass-adoption.
...
As said, "A picture is worth a thousand words"  and a running prototype worth a thousand pictures. So, we made a POC that you can check at http://investorx.io https://github.com/apper-tech/investorX (for more information, kindly check the readme file).
...
This DApp POC is also available at http://surveyx.io (and more information could also be found at its readme file).
...
The main idea is about the reward after the voting, in order to save the storage space of the blockchain and minimize the number of transactions by not deploying  non-essential transactions, we propose to communicate with voter (or maybe anonymous voter) off chain and implementing non repudiation is to ensure that a winner can claim the reward if it was not sent, in notice that we can validate the voting operation on-chain.
...
Kindly check the readme file at https://github.com/apper-tech/investorX for detailed information.
Actually, the POC is built with Angular, Truffle 5 & and solidity 0.5 and its code is available at GitHub. However, after being funded by Gnosis, we will re-implement it utilizing Gonsis Apollo packages. But we will keep it in Angular.

***SurveyX***

You can find helpful information in the readme file at https://github.com/apper-tech/surveyX.
Actually, the POC is built with React, Truffle 5 & and solidity 0.5 and its code is available at GitHub. However, after getting the grant from Gnosis we will re-implement it utilizing Gonsis Apollo packages; And we will keep it in React.

***NRX (NonRepudiationX) Scalability Solution***

What is nonrepudiation? It is the assurance that someone cannot deny the validity of something.  Non-repudiation could be achived with digital signature using the public-key/private-key of Ethereum external accounts (wallets).

The execution flow, that you can see in the next sequence diagram, is basically that we will ask both the user and the system-owner (the maker or an oracle) to sign, off-chain, their confirmation messages. Suchthat, the user will first sign his data off-chain.  And then the system-owner, as a confirmation, will off-chain sign the signed message he receives from the user. The user can then use the confirmation he received from the system-owner to claim his reward on-chain. Just like magic, the off-chain signature will be easily done because of the EIP712 standard that is  [already implemented in MetaMask](https://medium.com/metamask/eip712-is-coming-what-to-expect-and-how-to-use-it-bb92fd1a7a26 "already implemented in MetaMask").
...
Bellow is the _sequence diagram of NonRepudiationX:_ 

![sequence diagram](https://drive.google.com/uc?id=1tJQCMsgb9SpM7HSzoyTfXZQMWTkuxqD2)
...
Implementing Core Features
   - Deliverables
        ...
        - Truffle boxes means clear and clean code that fit for the purpose, but also generalized for other cases.
...
- Deliverables
    ...
    - Integrate NRX with Gnosis Apollo packages (we will fork the repositories and make pull requests for:
    - pm-contracts, pm-js, pm-trading-db and pm-trading-ui)
...
**Phase III:** 
 ...
 This phase is where we will look more for business achievements. And, this is only doable having a solid and sound solution. So, achieving the deliverables bellow, that could measure how much effect we could make in the market, is a proof of a good technical solution and a hard and smart work.
- Deliverables: Ensure a successful achievement to at least 5 of the following numbers:
    ...
    - 2000 viewers for YouTube video(s) in which we will explain about Gnosis, Gnosis Apollo, including our Truffle Boxes and who to use NonRepudiationX.
